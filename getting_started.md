...
This tutorial uses the [compose-scylladb-helloworld-nodejs](https://github.com/IBM-Cloud/compose-scylladb-helloworld-nodejs) sample app to demonstrate how to use Node.js to connect to a {{site.data.keyword.composeForScyllaDB_full}} service using the provided credentials. The application creates, reads from, and writes to a database using data supplied through the app's web interface.
{: shortdesc}
